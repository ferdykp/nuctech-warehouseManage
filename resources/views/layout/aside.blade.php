...
<aside id="sidebar" :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
    ...
    <nav class="space-y-1.5">
        ...
        <p class="px-4 mb-2 text-[10px] font-bold tracking-widest text-gray-500 uppercase">HR & Finance</p>
        ...
        <a href="{{ route('shift.index') }}"
            class="{{ $navItemClass }} {{ request()->routeIs('shift.index') ? $activeClass : '' }}">
            <i class="w-5 text-center fa-solid fa-clock-rotate-left"></i>
            <span>Shift</span>
        </a>
    </nav>
</aside>
        ...
        <form action="{{ route('sites.store') }}" method="POST" class="px-8 py-6 space-y-5">
            ...
            <div class="flex items-center justify-end gap-3 pt-4">
                <button type="button" @click="show = false"
                    class="px-5 py-2.5 text-sm font-semibold text-slate-600 hover:text-slate-800 transition-colors">Cancel</button>
                <button type="submit"
                    class="px-8 py-2.5 text-sm font-bold text-white bg-blue-600 rounded-xl shadow-lg shadow-blue-600/20 hover:bg-blue-700 active:scale-95 transition-all">Register
                    Machine</button>
            </div>
        </form>
    </div>
