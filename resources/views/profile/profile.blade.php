...
@section('content')
    <div class="w-full space-y-6">
        ...
        {{-- PROFILE HEADER CARD --}}
        <div class="p-6 bg-white border shadow-sm border-slate-200/80 rounded-2xl sm:rounded-3xl">
            <div class="flex flex-col items-center gap-5 sm:flex-row sm:items-start">
                <div class="relative w-20 h-20 sm:w-24 sm:h-24 shrink-0">
                    <img src="{{ asset('img/profile.png') }}" alt="Profile Picture"
                        class="object-cover w-full h-full border shadow-sm rounded-2xl border-slate-100">
                </div>
        ...
        <div class="overflow-hidden bg-white border shadow-sm border-slate-200/80 rounded-2xl sm:rounded-3xl">
            {{-- HEADER BAR --}}
            <div
                class="flex flex-col justify-between gap-3 px-6 py-4 border-b sm:flex-row sm:items-center border-slate-100 bg-slate-50/50">
                <div>
                    <h2 class="text-sm font-extrabold tracking-wider uppercase text-slate-800">User Account Details</h2>
                    <p class="text-xs text-slate-500">View personal credentials and permissions.</p>
                </div>
                <div class="flex items-center gap-2.5 shrink-0">
                    @if (Auth::user()->role === 'superadmin')
                        <a href="{{ route('profile.profileList') }}"
                            class="px-4 py-2 text-xs font-bold transition-all bg-white border text-slate-700 border-slate-200 rounded-xl hover:bg-slate-50 shadow-2xs active:scale-95">
                            <i class="mr-1 fa-solid fa-users-gear text-slate-400"></i> Manage Accounts
                        </a>
                    @endif

                    <a href="{{ route('profile.profileEdit', ['id' => auth()->id()]) }}"
                        class="px-4 py-2 text-xs font-bold text-white transition-all bg-blue-600 shadow-md rounded-xl hover:bg-blue-700 shadow-blue-600/20 active:scale-95">
                        <i class="mr-1 fa-solid fa-pen-to-square"></i> Edit Profile
                    </a>
                </div>
            </div>
            ...
            <div class="p-6 space-y-6 sm:p-8">
                <div class="space-y-4">
                    ...
                    <div class="grid items-center grid-cols-1 gap-2 py-3 border-b md:grid-cols-3 border-slate-100">
                        ...
                        <p class="text-sm font-bold md:col-span-2 text-slate-800">
                            {{ auth()->user()->name }}
                        </p>
                    </div>
                    ...
                    <div class="grid items-center grid-cols-1 gap-2 py-3 border-b md:grid-cols-3 border-slate-100">
                        ...
                        <p class="text-sm font-bold tracking-wide uppercase md:col-span-2 text-slate-800">
                            {{ str_replace('_', ' ', auth()->user()->role) }}
                        </p>
                    </div>
