...
@section('content')
                ...
                <div class="overflow-x-auto border rounded-lg">
                    <table class="w-full border-collapse">
                        ...
                        <tbody>
                                    ...
                                    @if (Auth::user()->role === 'admin')
                                        <td class="px-4 py-3 text-center">
                                            <div class="flex justify-center gap-2">
                                                <button type="button" onclick="openEditCategoryModal(this)" data-item="{{ json_encode($cat) }}"
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="p-4 text-center text-gray-500">Data kategori tidak ditemukan.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    {{-- MODAL CREATE --}}
    <div id="modal-create-category" class="fixed inset-0 z-50 flex items-center justify-center hidden px-4 bg-black/60 backdrop-blur-sm">
        <div class="relative w-full max-w-md overflow-hidden bg-white shadow-2xl rounded-2xl">
            ...
            <form action="{{ route('categories.store') }}" method="POST" class="p-6">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" required placeholder="Contoh: Mekanikal" class="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none">
                </div>
                <div class="mb-6">
                    <label class="block mb-1 text-sm font-semibold text-gray-700">Deskripsi</label>
                    <textarea name="description" rows="3" class="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none"></textarea>
                </div>
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="closeCreateCategoryModal()" class="px-5 py-2 text-sm font-semibold text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200">Batal</button>
                    <button type="submit" class="px-5 py-2 text-sm font-bold text-white bg-emerald-600 rounded-lg hover:bg-emerald-700">Simpan</button>
                </div>
            </form>
        </div>
    </div>
    ...
    {{-- MODAL EDIT --}}
    <div id="modal-edit-category" class="fixed inset-0 z-50 flex items-center justify-center hidden px-4 bg-black/60 backdrop-blur-sm">
        <div class="relative w-full max-w-md overflow-hidden bg-white shadow-2xl rounded-2xl">
            <div class="flex items-center justify-between px-6 py-4 border-b bg-gray-50">
            ...
            <form id="form-edit-category" method="POST" class="p-6">
                ...
                <div class="mb-4">
                    <label class="block mb-1 text-sm font-semibold text-gray-700">Nama Kategori</label>
                    <input type="text" id="edit_category_name" name="name" required class="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
                <div class="mb-6">
                    <label class="block mb-1 text-sm font-semibold text-gray-700">Deskripsi</label>
                    <textarea id="edit_category_description" name="description" rows="3" class="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"></textarea>
                </div>
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="closeEditCategoryModal()" class="px-5 py-2 text-sm font-semibold text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200">Batal</button>
                    <button type="submit" class="px-5 py-2 text-sm font-bold text-white bg-blue-600 rounded-lg hover:bg-blue-700">Update</button>
                </div>
            </form>
        </div>
    </div>
