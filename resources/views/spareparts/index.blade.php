...
@section('content')
    <div class="w-full px-6 py-6">
        <div class="bg-white shadow rounded-2xl">
            ...
            {{-- ACTION & FILTERS --}}
            <div class="p-6">
                <div class="flex flex-col gap-3 mb-4 w-full md:flex-row md:items-center md:justify-between">
                    <div class="flex items-center gap-2">
                        @if (Auth::user()->role === 'admin')
                            ...
                            <a href="{{ route('sparepart.export', $slug) }}"
                                class="flex items-center gap-2 p-3 text-sm font-semibold text-white transition-all bg-emerald-600 rounded-lg hover:bg-emerald-700">
            ...
            <form action="{{ route('sparepart.store', $slug) }}" method="POST" enctype="multipart/form-data" class="p-6">
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select name="category_id" class="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-green-500 outline-none">
    ...
    <div id="modal-import" class="fixed inset-0 z-50 flex items-center justify-center hidden bg-black/50 backdrop-blur-sm">
        <div class="bg-white p-6 rounded-2xl w-full max-w-md shadow-2xl">
            <div class="flex items-center justify-between mb-4">
                ...
                <button onclick="closeImportModal()" class="text-gray-400 hover:text-red-500 transition-colors">
                    <i class="fa-solid fa-xmark text-xl"></i>
                </button>
            </div>
            <form id="form-import" action="{{ route('sparepart.import', $slug) }}" method="POST" enctype="multipart/form-data">
                @csrf
                {{-- Drop Zone --}}
                <div id="import-dropzone" class="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 rounded-xl cursor-pointer hover:border-blue-400 hover:bg-blue-50/50 transition-all">
                    <i id="dropzone-icon" class="fa-solid fa-cloud-arrow-up text-3xl text-gray-400 mb-2"></i>
                    <span id="dropzone-text" class="text-sm text-gray-500 font-medium">Klik atau drag file ke sini</span>
                    <span id="dropzone-hint" class="text-xs text-gray-400 mt-1">.xlsx / .xls / .csv — max 10MB</span>
                    <input type="file" id="import-file-input" name="file" class="hidden" accept=".xlsx,.xls,.csv">
                </div>

                {{-- Submit Button (hidden until file selected) --}}
                <button type="submit" id="btn-submit-import" class="hidden w-full mt-4 px-5 py-3 text-sm font-bold text-white bg-blue-600 rounded-xl hover:bg-blue-700 shadow-lg shadow-blue-100 transition-all">
                    <i class="fa-solid fa-file-import mr-2"></i> Submit & Import Data
                </button>
            </form>
            <div class="mt-4 p-3 bg-blue-50 rounded-lg text-xs text-blue-700 leading-relaxed">
                <p class="font-semibold mb-1"><i class="fa-solid fa-circle-info mr-1"></i> Format yang didukung:</p>
    ...
    function showImportToast(message, type = 'info', duration = 0) {
        ...
        const icons = {
            info:    '<i class="fa-solid fa-spinner fa-spin mr-2"></i>',
            success: '<i class="fa-solid fa-circle-check mr-2"></i>',
            error:   '<i class="fa-solid fa-circle-xmark mr-2"></i>',
        };

        toast.className = `px-5 py-4 text-sm font-medium border shadow-xl rounded-xl transition-all duration-500 ${styles[type]} transform translate-y-3 opacity-0 scale-95`;
    ...
    fileInput.addEventListener('change', function() {
        ...
        document.getElementById('dropzone-icon').className = 'fa-solid fa-file-excel text-3xl text-green-500 mb-2';
    ...
    document.getElementById('form-import').addEventListener('submit', function(e) {
        ...
        xhr.addEventListener('load', function() {
            ...
            try {
                ...
                if (xhr.status === 422) {
                    showImportToast('File tidak valid. Pastikan format .xlsx/.xls/.csv dan max 10MB.', 'error', 6000);
                } else {
    ...
    {{-- MODAL EDIT --}}
    <div id="modal-edit" class="fixed inset-0 z-50 flex items-center justify-center hidden px-4 bg-black/60 backdrop-blur-sm">
        <div class="relative w-full max-w-2xl overflow-hidden bg-white shadow-2xl rounded-2xl">
            <div class="flex items-center justify-between px-6 py-4 border-b bg-gray-50">
            ...
            <form id="form-edit" method="POST" enctype="multipart/form-data" class="p-6">
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                        <label class="block mb-1 text-sm font-semibold text-gray-700">Item Name</label>
                        <input type="text" id="edit_item_name" name="item_name" required class="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                    </div>
                    <div>
                        <label class="block mb-1 text-sm font-semibold text-gray-700">Kategori</label>
                        <select id="edit_category_id" name="category_id" class="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                            <option value="">-- Pilih Kategori --</option>
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}">{{ $category->name }}</option>
                            @endforeach
                    ...
                    <div>
                        <label class="block mb-1 text-sm font-semibold text-gray-700">Serial Number</label>
                        <input type="text" id="edit_serial_number" name="serial_number" class="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                    </div>
                    <div>
                        <label class="block mb-1 text-sm font-semibold text-gray-700">Type / Model</label>
                        <input type="text" id="edit_type" name="type" required class="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                    </div>
                    <div>
                        <label class="block mb-1 text-sm font-semibold text-gray-700">Satuan (UOM)</label>
                        <select id="edit_uom" name="uom" required class="w-full p-2.5 border rounded-lg outline-none">
                            <option value="PCS">PCS</option>
                            <option value="SET">SET</option>
                            <option value="UNIT">UNIT</option>
                        </select>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="edit_note" name="note" rows="2" class="w-full p-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"></textarea>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block mb-1 text-sm font-semibold text-gray-700">Update Image (Opsional)</label>
                        <input type="file" name="image" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                    </div>
                </div>
                <div class="flex justify-end gap-3 mt-8">
                    <button type="button" onclick="closeEditModal()" class="px-6 py-2.5 text-sm font-semibold text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200">Batal</button>
                    <button type="submit" class="px-6 py-2.5 text-sm font-bold text-white bg-blue-600 rounded-lg hover:bg-blue-700">Update Sparepart</button>
                </div>
            </form>
        </div>
    </div>
