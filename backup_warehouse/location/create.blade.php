...
@section('content')
    <div class="w-full px-6 py-3">
        <div class="overflow-hidden bg-white shadow-2xl rounded-2xl">
            ...
            <div class="px-8 py-6 border-b bg-gray-50">
                ...
                <p class="mt-1 text-sm text-gray-500">
                    Form input data sparepart untuk lokasi Semarang
                </p>
        ...
        @error('machine_type') border-red-500 @enderror"
                            required>
                            <option value="" disabled {{ old('machine_type') ? '' : 'selected' }}>
                                -- Pilih Machine Type --
                            </option>

                            <option value="fs6000jkt" {{ old('machine_type') == 'fs6000jkt' ? 'selected' : '' }}>
                                FS6000 Jakarta
                            </option>
                            <option value="fs6000sby" {{ old('machine_type') == 'fs6000sby' ? 'selected' : '' }}>
                                FS6000 Surabaya
                            </option>
                            <option value="fs6000smg" {{ old('machine_type') == 'fs6000smg' ? 'selected' : '' }}>
                                FS6000 Semarang
                            </option>
                            <option value="ebeam" {{ old('machine_type') == 'ebeam' ? 'selected' : '' }}>
                                E-Beam
                            </option>
                            <option value="ctmic" {{ old('machine_type') == 'ctmic' ? 'selected' : '' }}>
                                CTMIC
                            </option>
                        </select>
