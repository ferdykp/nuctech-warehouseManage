...
@section('content')
    <div class="w-full py-3">
        <div class="max-w-6xl mx-auto overflow-hidden bg-white shadow-xl rounded-2xl">
            ...
            {{-- BODY --}}
            <div class="p-6">
                <form action="{{ route('fsjkt.update', $fs6000smg->id) }}" method="POST" enctype="multipart/form-data"
                    class="space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <div class="grid gap-5 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <input type="text" name="item_name" value="{{ old('item_name', $fs6000smg->item_name) }}"
                            ...
                            <div>
                                ...
                                <input type="text" name="type" value="{{ old('type', $fs6000smg->type) }}"
                                    class="w-full px-4 py-2 border rounded-lg focus:ring focus:ring-blue-200 focus:outline-none
                            ...
                            <div class="grid gap-5 md:grid-cols-3 md:col-span-2">
                                ...
                                <div>
                                    ...
                                    <input type="number" name="stock" value="{{ old('stock', $fs6000smg->stock) }}"
                                        class="w-full px-4 py-2 border rounded-lg focus:ring focus:ring-blue-200 focus:outline-none
                                ...
                                <div>
                                    ...
                                    @error('uom') border-red-500 @enderror"
                                        ...
                                        <option value="pcs" {{ old('uom', $fs6000smg->uom) == 'pcs' ? 'selected' : '' }}>
                                            Pieces</option>
                                        <option value="set" {{ old('uom', $fs6000smg->uom) == 'set' ? 'selected' : '' }}>
                                            Set</option>
                                        <option value="meter"
                                            {{ old('uom', $fs6000smg->uom) == 'meter' ? 'selected' : '' }}>Meter</option>
                                        <option value="liter"
                                            {{ old('uom', $fs6000smg->uom) == 'liter' ? 'selected' : '' }}>Liter</option>
                                        <option value="pack"
                                            {{ old('uom', $fs6000smg->uom) == 'pack' ? 'selected' : '' }}>Pack</option>
                                    </select>
                                    @error('uom')
                                ...
                                <div>
                                    ...
                                    <input type="date" name="date_update"
                                        value="{{ old('date_update', $fs6000smg->date_update) }}"
                                        class="w-full px-4 py-2 border rounded-lg focus:ring focus:ring-blue-200 focus:outline-none
                                ...
                                @error('location') border-red-500 @enderror">
                                    ...
                                    @foreach ($locations as $loc)
                                        <option value="{{ $loc->location_name }}"
                                            {{ old('location', $fs6000smg->location) == $loc->location_name ? 'selected' : '' }}>
                                            {{ $loc->location_name }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div>
                        ...
                        <div class="mb-5">
                            ...
                            <textarea name="note" rows="3"
                                class="w-full px-4 py-2 border rounded-lg focus:ring focus:ring-blue-200 focus:outline-none
                            @error('note') border-red-500 @enderror">{{ old('note', $fs6000smg->note) }}</textarea>
                            @error('note')
                                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            @if ($fs6000smg->image)
                                <img src="{{ asset('storage/' . $fs6000smg->image) }}"
